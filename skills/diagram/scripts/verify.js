...
(async () => {
  ...
  try {
    new Function('document', 'window', script.replace(/render\([^)]*\);?\s*$/, ''));
    ok.push('JS compiles');
  } catch (e) {
    problems.push('JS syntax error: ' + e.message);
  }

  // 3. extract NODES, ZONE_LABELS, SC + layout metrics
  function grab(a, b) {
    const i = html.indexOf(a);
    return i < 0 ? '' : html.slice(i, html.indexOf(b, i));
  }
  let NODES, SC, COLW, COLGAP, ROWH, ROWGAP;
  try {
    const metrics = html.match(
      /const\s+COLW\s*=\s*(\d+)\s*,\s*COLGAP\s*=\s*(\d+)\s*,\s*ROWH\s*=\s*(\d+)\s*,\s*ROWGAP\s*=\s*(\d+)\s*;/
    );
    ...
    const code =
      objBlock('const NODES =') + '\n' + objBlock('const SC =') + '\n' + 'return { NODES, SC };';
    ({ NODES, SC } = new Function(code)());
    ok.push(`parsed ${Object.keys(NODES).length} nodes, ${Object.keys(SC).length} scenarios`);
  } catch (e) {
    problems.push('could not parse NODES/SC: ' + e.message);
  }

  if (NODES && SC) {
    const COL_X = (c) => 120 + c * (COLW + COLGAP),
      ROW_Y = (r) => 220 + r * (ROWH + ROWGAP);
    let LIVE = {};
    const layout = (grid) => {
      LIVE = {};
      for (const id in grid) {
        ...
        const n = NODES[id];
        if (!n) {
          problems.push(`grid references unknown node "${id}"`);
          continue;
        }
        LIVE[id] = {
          x: COL_X(c) + (COLW - n.w) / 2,
          y: ROW_Y(r) + (ROWH - n.h) / 2,
          w: n.w,
          h: n.h
        };
      }
    };
    const port = (id, side, frac = 0.5) => {
      const n = LIVE[id];
      if (!n) return { x: 0, y: 0 };
      if (side === 'r') return { x: n.x + n.w, y: n.y + n.h * frac };
      if (side === 'l') return { x: n.x, y: n.y + n.h * frac };
      if (side === 't') return { x: n.x + n.w * frac, y: n.y };
      return { x: n.x + n.w * frac, y: n.y + n.h };
    };
    const pp = (s) => {
      const [id, side, frac] = s.split(':');
      return port(id, side, frac !== undefined ? +frac : 0.5);
    };
    const bez = (a, cx, cy, b, t) => {
      const u = 1 - t;
      return {
    ...
    const curveHitsNode = (a, b, bow, ex) => {
      const dx = b.x - a.x,
        dy = b.y - a.y,
      ...
      for (const id in LIVE) {
        ...
        for (let t = 0.03; t <= 0.97; t += 0.02) {
          const p = bez(a, cx, cy, b, t);
          if (p.x > n.x - 6 && p.x < n.x + n.w + 6 && p.y > n.y - 6 && p.y < n.y + n.h + 6)
            return id;
        }
      }
      return null;
    ...
    const resolve = (flow) => {
      const items = flow.map((f, i) => {
        const o = Object.assign({}, f[3]);
        const net = o.net || 'http',
          ep = o.ep || '';
        if (net === 'compute') {
          const node = LIVE[f[0].split(':')[0]];
          const cxn = node ? node.x + node.w / 2 : 0,
            cyn = node ? node.y + node.h + 70 : 0;
          const t = i + 1 + '. compute';
          const w = Math.max(t.length + 1, ep.length) * CHARW + 28,
            h = ep ? 50 : 30;
          return { compute: true, lcx: cxn, lcy: cyn, w, h };
        }
        const a = pp(f[0]),
          b = pp(f[1]),
          n = i + 1;
        ...
        const t = n + '. ' + (NETLABEL[net] || net);
        const w = Math.max(t.length + 1, ep.length) * CHARW + 28,
          h = ep ? 50 : 30;
        return { lcx: lp.x, lcy: lp.y, w, h };
      });
      const colliding = (i) => {
        const it = items[i],
          r = { x: it.lcx - it.w / 2, y: it.lcy - it.h / 2, w: it.w, h: it.h };
        ...
        for (let j = 0; j < items.length; j++) {
          if (j === i) continue;
          const o = items[j],
            ro = { x: o.lcx - o.w / 2, y: o.lcy - o.h / 2, w: o.w, h: o.h };
          if (rectsOverlap(r, ro)) {
            hit = true;
            let vx = it.lcx - o.lcx,
              vy = it.lcy - o.lcy;
            if (vx === 0 && vy === 0) vy = -1;
      ...
      for (let p = 0; p < 400; p++) {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (c.hit) {
            ...
            items[i].lcx += (c.fx / m) * 8;
            items[i].lcy += (c.fy / m) * 8 - 1;
            moved = true;
          }
        }
        if (!moved) break;
    ...
    for (const key of Object.keys(SC)) {
      const sc = SC[key];
      if (!sc.grid || !sc.flow || !sc.steps) {
        problems.push(`${key}: missing grid/flow/steps`);
        continue;
      }
      layout(sc.grid);
      const ids = Object.keys(sc.grid);
      // node overlaps
      for (let i = 0; i < ids.length; i++)
        for (let j = i + 1; j < ids.length; j++) {
          ...
          if (
            ...
            problems.push(`${key}: nodes "${ids[i]}" & "${ids[j]}" overlap`);
        }
      // arrows through nodes
      sc.flow.forEach((f, i) => {
        if ((f[3] || {}).net === 'compute') return;
        const h = curveHitsNode(pp(f[0]), pp(f[1]), (f[3] || {}).bow || 0, [
          f[0].split(':')[0],
          f[1].split(':')[0]
        ]);
        if (h)
          problems.push(
            `${key}: arrow ${i + 1} (${f[0].split(':')[0]}→${f[1].split(':')[0]}) passes through "${h}"`
          );
      });
      // labels
      ...
      for (let i = 0; i < items.length; i++) {
        const it = items[i],
          r = { x: it.lcx - it.w / 2, y: it.lcy - it.h / 2, w: it.w, h: it.h };
        if (!it.compute && rectHitsNode(r)) problems.push(`${key}: label ${i + 1} sits on a node`);
        for (let j = i + 1; j < items.length; j++) {
          const o = items[j],
            ro = { x: o.lcx - o.w / 2, y: o.lcy - o.h / 2, w: o.w, h: o.h };
          if (rectsOverlap(r, ro)) problems.push(`${key}: labels ${i + 1} & ${j + 1} overlap`);
        }
      }
      // structural
      if (sc.flow.length !== sc.steps.length)
        problems.push(`${key}: flow has ${sc.flow.length} arrows but steps has ${sc.steps.length}`);
      sc.flow.forEach((f, i) => {
        const aret = (f[3] || {}).ret ? 1 : 0;
        const sret = sc.steps[i] ? (sc.steps[i][1] ? 1 : 0) : 0;
        if (aret !== sret)
          problems.push(`${key}: step ${i + 1} ret flag mismatch (arrow=${aret} step=${sret})`);
      });
      const touched = new Set();
      sc.flow.forEach((f) => {
        touched.add(f[0].split(':')[0]);
        touched.add(f[1].split(':')[0]);
      });
      ids.forEach((id) => {
        if (!touched.has(id)) problems.push(`${key}: node "${id}" in grid but no arrow touches it`);
      });
    }
  }
  ...
  async function main() {
    ...
    if (rc.skipped) {
      ...
      if (rc.pageErrors.length)
        rc.pageErrors.forEach((e) => problems.push('RUNTIME ERROR during render: ' + e));
      // each scenario should render arrows if it has steps (>1 step implies flows exist)
      rc.tabs.forEach((tb, i) => {
        if (tb.steps > 0 && tb.flowItems === 0)
          problems.push(
            `scenario tab ${i} ("${tb.title || 'untitled'}") rendered 0 arrows despite ${tb.steps} steps — likely a malformed arrow crashed render`
          );
        if (!tb.title)
          problems.push(
            `scenario tab ${i} has an EMPTY title (render produced no title — scenario likely crashed)`
          );
      });
      if (!rc.pageErrors.length)
        ok.push(`render check passed (${rc.tabs.length} tabs rendered, no runtime errors)`);
    }
